...
// by the Python alert engine. Keep the display mapping here rather than making
// users type implementation codes into a form.
...
] as const satisfies ReadonlyArray<{ value: CalendarEventType; label: string }>;

export const CALENDAR_EVENT_TYPES = CALENDAR_EVENT_TYPE_OPTIONS.map((option) => option.value) as CalendarEventType[];
...
export function isKnownCalendarEventType(value: string): value is CalendarEventType {
